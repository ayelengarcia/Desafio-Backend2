...
class ProductManager {
  ...
  addProduct = async (title, description, price, thumbnail) => {
   ...
   const newProduct = {
    ...
    if (!title || !description || !price || !thumbnail) return console.log("Todos los campos son obligatorios.");

    const idExiste = productosAll.some((producto) => producto.id === newProduct.id);
    if (idExiste) {
  ...
  deleteProduct = async(productId)=> {
    try {
      const productosAll = await this.getProducts();
      const productIndex = productosAll.findIndex((product) => product.id === productId);
...
async function run () {
    ...
    console.log("Actualizado:", await producto.updateProduct(7, {
        "title": "Lala2",
        "description": "Campo2",
        "price": 990,
        "thumbnail": "#",
